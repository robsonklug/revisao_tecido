...
with st.sidebar:
    ...
    # Limpeza da tela central ao trocar produto (sem apagar o histórico geral)
...
with col_in:
    ...
    # Padronização da chave para evitar KeyError: sempre "Pts"
...
with col_tab:
    if st.session_state.defeitos_atuais:
        ...
        st.table(df_obs)
        total_pontos = df_obs["Pts"].sum() # Agora a chave está garantida
    else:
...
if st.button("FINALIZAR E ARMAZENAR REVISÃO"):
    # Salva o estado atual no histórico global
    registro = {
    ...
    st.success(f"Revisão do produto {cod_selecionado} salva no histórico global!")

# Exibe todas as revisões feitas na sessão
if st.session_state.historico_acumulado:
    st.write("---")
    st.write("### Histórico Completo de Revisões")
